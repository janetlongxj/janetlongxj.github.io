const cmt=getElement("comments"),{i18nReplies,i18nReblogs,i18nFavourites,i18nLoading,i18nErr,i18nNoComment}=cmt.dataset,cmtSty=document.createElement("style");cmtSty.textContent=`
    ...
    }`,document.head.appendChild(cmtSty);const fedRoot=getElement("fed-comments");if(fedRoot)var mstdCommentsLoaded,bskyCommentsLoaded,skeetURL,replies=0,reblogs=0,favourites=0;const addToCounter=(e,t,n)=>{replies=replies+e,reblogs=reblogs+t,favourites=favourites+n},renderStat=(e,t,n,s)=>`
    <a class='${s} ${e>0?"active":""}' href='${t}' rel='external noreferrer nofollow' aria-label='${n}'>
        <span>${e>0?e:""}</span>
    </a>
`,respondToVisibility=(e,t)=>{const n=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&t()})});n.observe(e)},getURI=(e,t)=>{const n=e.split("/"),s=n[0]==="https:"||n[0]==="http:";return s?t(n):e},checkResponseStatus=e=>{if(!e.ok)throw new Error(`HTTP error, status = ${e.status}`)},mstdRoot=getElement("mstd-comments");if(mstdRoot){mstdCommentsLoaded=!1;const e=mstdRoot.dataset.url,t=e.split("/")[4],r=e=>`https://${e[2]}/api/v1/statuses/${e[4]}`,n=getURI(e,r);if(n!==e){const e=async()=>{if(mstdCommentsLoaded)return;fedRoot||(mstdRoot.innerHTML=`<span id=mstdIsLoading class=loading>${i18nLoading}</span>`);try{const[r,c]=await Promise.all([fetch(n),fetch(n+`/context`)]),[e,s]=await Promise.all([r.json(),c.json()]);checkResponseStatus(r),checkResponseStatus(c),fedRoot?addToCounter(e.replies_count,e.reblogs_count,e.favourites_count):(getElement("stats").innerHTML=a(e),getElement("mstdIsLoading").remove()),getElement("discussion-starter-content").innerHTML=i(e),s.descendants?.length>0?typeof DOMPurify!="undefined"?DOMPurify.sanitize(o(s.descendants,t),{RETURN_DOM_FRAGMENT:!0}):o(s.descendants,t):fedRoot||(mstdRoot.innerHTML=i18nNoComment),mstdCommentsLoaded=!0,mstdRoot.setAttribute("aria-busy","false")}catch(e){console.error(`Mastodon ${i18nErr}`,e),mstdRoot.innerHTML=`Mastodon ${i18nErr} : ${e}`}};respondToVisibility(mstdRoot,e)}const i=e=>{const t=e.media_attachments.length>0?`<div class='attachments'>${e.media_attachments.map(c).join("")}</div>`:"";return`
    ...
    `,l=t=>{const n=new Date(t.post.record.createdAt);return`
        ...
        </li>`},t=e=>{const n=document.createDocumentFragment(),s=e=>{const t=document.createElement("li");return t.innerHTML=e.trim(),t.firstChild};for(const o of e.replies){const i=s(l(o));if(o.replies.length>0){const e=document.createElement("ul");i.appendChild(e).appendChild(t(o))}n.appendChild(i)}return n}}const sortComment=e=>{const t=Array.from(e);if(t.length>0){const e=new Set;t.sort(({dataset:{date:e}},{dataset:{date:t}})=>e.localeCompare(t)).forEach(t=>{e.has(t.id)?t.remove():(e.add(t.id),t.parentNode.appendChild(t))})}else e.parentNode.innerHTML=i18nNoComment},aggregateComment=()=>{if(mstdCommentsLoaded&&bskyCommentsLoaded){const e=getElements("#fed-comments > li[data-date]");sortComment(e),getElement("stats").innerHTML=`
